...
//------------------------------------------------------------------
int run_dmrg(int is_my_initial_state, int Nx, int Ny, int Nup, int Ndown, bool yperiodic, 
    ...
    string one_particle_corr_file_name)
{
    ...
    double EnergyConvergenceThreshold = 1e-5;                     
    ...
    energy_file << "W="<< Ny << "L="<< Nx << "Nup =" << Nup << " Ndown = " << Ndown << " E = " << fixed << energy << " time: "
        ...
        << " sec" << " t_up = " << t_parallel_up << "t_down = " << t_parallel_dn
	<< " is_my_inititalization: "<< is_my_initial_state << endl;
    energy_file.close();
...
int main(int argc, char *argv[]){
    ...
    int add_up = atoi(argv[7]);
    int	add_down = atoi(argv[8]);    
    // double cutoff = atof(argv[7]);
    // double converge = atof(argv[8]);
    
    int is_my = atoi(argv[9]);
    ...
    run_dmrg(is_my,Nx,Ny,Nup,Ndown,false,1.0,1.0,t_down,t_down,U, energy_file_name, superconductive_corr_file_name, "W1_40_point.txt");
    // }
    // else{
    // cout << "boo";
// }    	
